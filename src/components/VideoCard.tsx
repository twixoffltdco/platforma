      ...
      <div className="flex gap-3 px-1">
        ...
        <div className="flex flex-col flex-1 min-w-0">
          <Link to={`/watch/${video.id}`}>
            <h3 className="text-white font-semibold text-[15px] leading-tight line-clamp-2 group-hover:text-blue-400 transition-colors">
              {video.title}
            </h3>
          </Link>
          <div className="mt-1 flex flex-col">
            <Link to={`/channel/${video.author}`} className="text-gray-400 text-sm hover:text-white flex items-center gap-1">
              {video.author}
